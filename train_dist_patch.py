...
GPU_CHECKPOINT, GPU_LOG = [x % torch.cuda.device_count() for x in range(2)]

def do_work(rank, args):
    ...
    def train(epoch):
        ...
        for X, y in tqdm.tqdm(train_loader):
            input = X.to(rank)
            target = y.to(rank).to(torch.float32)
    ...
    def small_certify(epoch, k=25):
        ...
        res = calculate_radii(test_loader.dataset, range(k), model, 10**2, 10**4,
            1e-3, args.sigma, device=rank, batch_size=batch_size*2)
